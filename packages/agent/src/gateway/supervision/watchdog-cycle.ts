...
export async function runGatewayWatchdogPlatform(params: {
  ...
  try {
    ...
    // Contain any failure (e.g. adapter.health()/watch() throwing) to this
    // platform so the watchdog loop continues supervising the others.
    const detail = `${platform} watchdog cycle failed during ${reason}: ${error instanceof Error ? error.message : String(error)}`;
    return [applySupervisionOutcome(deps, platform, "recover", detail)];
  }
}
