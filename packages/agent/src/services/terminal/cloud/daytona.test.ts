...
describe("daytona cloud backend", () => {
  const roots = new Set<string>();

  it("returns a preview and missing-target run failure", async () => {
    const root = mkdtempSync(join(tmpdir(), "doolittle-daytona-cloud-"));
    roots.add(root);
    const store = new CloudStoreManager(join(root, "cloud-sessions.json"));
    const backend = createDaytonaExecutionBackend(store);
    ...
    expect(preview.detail).toContain("Daytona execution uses");
    ...
    const result = await backend.run("printf ok", {
      ...
      settings: {
        ...settings,
        execution: { ...settings.execution, daytonaTarget: "" },
      },
    });
