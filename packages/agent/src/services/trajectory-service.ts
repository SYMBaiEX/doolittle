...
interface TrajectoryBenchmarkCaseInput {
  manifestPath?: string;
  label?: string;
  purpose?: string;
  tags?: string[];
...
export class TrajectoryService {
  ...
  describeBenchmarkEnvironment(): TrajectoryBenchmarkEnvironmentSummary {
    const latestBundle = this.listBundles(1)[0];
    ...
    return {
      ...
      bundleCount: this.listBundles(100).length,
  ...
  createBenchmarkManifest(input: {
    ...
    const label = this.slug(input.label ?? "trajectory-benchmark");
    const stamp = Date.now();
    const manifestPath = join(
      this.baseDir,
    ...
    const bundles = this.listBundles(200);
    const environment = this.describeBenchmarkEnvironment();
    const cases = input.cases.map((entry, index) => {
      ...
      const bundle = this.describeBundle(resolvedPath);
      return {
  ...
  async runBenchmark(manifestPath: string): Promise<TrajectoryBenchmarkRun> {
    ...
    for (const [index, entry] of manifest.cases.entries()) {
      const replay = this.replayBundle(entry.manifestPath);
      const evaluation = await this.evaluateBundle(entry.manifestPath, {
    ...
    const stamp = Date.now();
    const label = this.slug(`${manifest.label}-run`);
    const reportPath = join(
      this.baseDir,
  ...
  async runLatestBenchmark(): Promise<TrajectoryBenchmarkRun | undefined> {
    const manifests = this.listBenchmarkManifests(20);
    const latest = manifests[0];
    if (!latest) {
      return undefined;
    }
    return this.runBenchmark(latest.manifestPath);
  }
  ...
  // -------------------------------------------------------------------------
  // RL-ready standardized exports (Hermes-compatible schema)
  // -------------------------------------------------------------------------

  /**
   * Export a session as an RL-ready JSONL file using a standardized schema
   * compatible with the Hermes Agent trajectory format. Each line is a
