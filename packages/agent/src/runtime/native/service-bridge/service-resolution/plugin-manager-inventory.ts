...
function normalizeRuntimePluginState(value: unknown) {
  ...
  const name =
    asNonEmptyString(state.name) ??
    asNonEmptyString(plugin?.name) ??
    asNonEmptyString(state.id) ??
    "unknown-plugin";
  const status = asNonEmptyString(state.status) ?? "loaded";
  const source =
  ...
  return {
    id: asNonEmptyString(state.id) ?? name,
    name,
    status,
    enabled: status === "ready" || status === "loaded",
    source,
    description: asNonEmptyString(plugin?.description),
