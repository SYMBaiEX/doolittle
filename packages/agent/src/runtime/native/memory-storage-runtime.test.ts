...
import { createMemoryStorageRuntimeService } from "./memory-storage-runtime";
...
async function createProvider(root: string): Promise<MemoryStorageProvider> {
  const ServiceClass = createMemoryStorageRuntimeService(
...
describe("Eliza memory storage provider", () => {
