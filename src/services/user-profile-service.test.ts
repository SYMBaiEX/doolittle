...
describe("UserProfileService", () => {
  ...
  it("extracts aliases, goals, tools, and work style signals", () => {
    const root = mkdtempSync(join(tmpdir(), "eliza-agent-user-profile-signals-"));
    const service = new UserProfileService(root);

    try {
      service.observe("user-2", "You can call me AJ.", "telegram");
      service.observe("user-2", "I want to ship the Docker backend this week.", "telegram");
      service.observe("user-2", "I usually use Bun with Docker and Lightpanda.", "telegram");
      service.observe("user-2", "I work best with concise step-by-step updates.", "telegram");

      const profile = service.get("user-2");
      expect(profile.aliases).toContain("AJ");
      expect(profile.goals?.some((entry) => entry.includes("ship the Docker backend"))).toBe(true);
      ...
      expect(profile.workStyle?.some((entry) => entry.includes("concise step-by-step"))).toBe(true);
