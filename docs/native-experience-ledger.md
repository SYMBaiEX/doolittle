...
| Stateful OpenAI-compatible Responses API transport | `gateway/platforms/api_server.py` | `packages/agent/src/services/api-transport-service.ts`, `packages/agent/src/server/routes/conversation/responses.ts` | Mostly covered | Doolittle persists `previous_response_id`, reuses one stable response ID across persistence and streaming, accepts text content blocks, and emits ordered semantic lifecycle, item, part, delta, done, completed, and failed events. Deeper conversation-object semantics and multimodal input remain. |
