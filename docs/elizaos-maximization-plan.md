...
> Date: 2026-03-23
> Last validated against the Eliza source tree and npm artifacts: 2026-08-06

> **Release audit:** Doolittle remains on the coherent npm `beta` train,
> `2.0.3-beta.7`. The semver-higher `2.0.11-beta.7` artifacts were published
> earlier, do not map to the later beta.7 source tag, and include an unusable
> agent-orchestrator package without runtime code. They are not an upgrade.
...
**What it is:** Policy-bounded code execution through the installed Eliza `SandboxManager` and `runShell` primitives. Doolittle's `@doolittle/plugin-local-sandbox`, consolidated inside `doolittle-plugin`, adapts those primitives to the product's local execution and autocoder receipts.

**Where it lives:**
- Doolittle adapter: `packages/plugins/doolittle-plugin/local-sandbox`
- Native execution primitives: `@elizaos/agent/services/sandbox-manager`
- Research integration: `ResearchCodeInterpreterTool` in model types (`{ type: "code_interpreter", container: { type: "auto" } }`)

**Current decision:**
1. Keep the existing Eliza-native sandbox-manager adapter as the execution owner.
2. Do not add `@elizaos/plugin-e2b-sandbox` or `@elizaos/plugin-native-filesystem`: neither is published on the compatible npm beta train, and current source-only implementations are not a drop-in replacement for Doolittle's repository, receipt, and approval policy.
3. Use the provider-owned code-interpreter tool for RESEARCH model calls; it is a remote model capability, not the same lifecycle as the local repository sandbox.
4. Reconsider a cloud sandbox only after Eliza publishes a coherent package with explicit filesystem containment, approval, teardown, and execution-receipt contracts.

**Expected impact:** Agent-generated code remains behind the runtime's established containment and receipt policy without introducing a second, incompatible sandbox owner.
...
| **Medium** | Triggers, Sandbox, Heartbeat, Tool hooks, MCP/Skills, Validation, Sampling | 10-14 | Compatible published cloud sandbox only if explicitly required; MCP servers configured |
