...
background mode. Update checks are also explicit. macOS relies on the platform's
signed and notarized update artifact, while Windows binds `app-update.yml` to
the exact Authenticode signer subject before a tagged artifact is accepted.
Linux automatic installation is disabled until the release feed has an
independently pinned metadata signature; Linux users download and verify a
release manually. The renderer never selects an arbitrary update origin.
...
```bash
nub run check:release
```

This includes source and desktop verification, acceptance checks, a critical
repository audit, a coverage check that stops on newly published unreviewed
high advisories, and a fresh packaged-runtime inventory audit. Every native
producer repeats that runtime policy check against the actual bundled manifest;
high-severity versions in the shipped runtime are release blockers even when a
broader development-only Eliza dependency is not part of the desktop artifact.
Runtime preparation also emits `runtime/bin/THIRD-PARTY-NOTICES.txt`, with
declared license metadata and license text for exactly the emitted bundle and
...
`@lydell/node-pty-darwin-arm64@1.1.0` similarly uses the MIT text from its
published sibling `@lydell/node-pty`.
The shipped `@elizaos/*@2.0.3-beta.7` train likewise uses the MIT text from its
published sibling `@elizaos/core`; a version change is not implicitly allowed.
When an emitted package has no metadata field but has package-local text, only
the unambiguous `MIT License` header is accepted as an attributable MIT license.
The distributable desktop runtime also fails closed when any emitted dependency
declares GPL or AGPL terms. The optional Telegram and WhatsApp native plugin
closures remain available to source/CLI installations but are compiled out of
the MIT desktop artifact. Discord remains available; its optional voice helper
uses a system `ffmpeg`/`avconv` binary instead of bundling `ffmpeg-static`.
Checked-in canonical GPL text remains only as a license-generator regression
fixture and is not evidence that a GPL dependency may ship in the desktop.
`@puppeteer/browsers@3.2.1` is similarly mapped to the checked-in canonical
...
`WIN_CSC_LINK` and `WIN_CSC_KEY_PASSWORD`, plus the public repository variable
`WIN_PUBLISHER_NAME` containing the certificate's complete subject DN. The
Windows workflow passes the credentials directly to electron-builder, writes
that exact subject into updater metadata, and requires valid matching
Authenticode signatures on both `Doolittle.exe` and the NSIS installer before
upload.
