...
export const COMPOSER_PROJECT_GLYPH_CLASS =
  "grid size-5 shrink-0 place-items-center rounded-md border border-[color-mix(in_srgb,var(--composer-project-color,var(--accent))_28%,var(--border))] bg-[color-mix(in_srgb,var(--composer-project-color,var(--accent))_10%,var(--surface-soft))] font-mono text-[9px] font-bold text-[var(--composer-project-color,var(--accent))]";
...
export const COMPOSER_POPOVER_HEADER_CLASS =
  "flex items-start border-[var(--border)] border-b px-3.5 pt-3.25 pb-2.5 [&>span]:grid [&>span]:gap-0.5 [&_small]:text-[9px] [&_small]:text-[var(--faint)] [&_strong]:text-[11px] [&_strong]:font-semibold [&_strong]:text-[var(--text)]";
