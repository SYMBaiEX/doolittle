...
export const REVIEW_COMMAND_CLASS =
  "review-command grid gap-2 [&>span]:font-[var(--font-mono)] [&>span]:text-[10px] [&>span]:font-bold [&>span]:tracking-[0.1em] [&>span]:text-[var(--accent)] [&>span]:uppercase [&_code]:m-0 [&_code]:overflow-auto [&_code]:whitespace-pre-wrap [&_code]:rounded-[var(--radius-sm)] [&_code]:border [&_code]:border-[var(--border-strong)] [&_code]:bg-[#080706] [&_code]:px-4 [&_code]:py-3.5 [&_code]:text-xs [&_code]:leading-[1.55] [&_code]:text-[var(--text)]";
...
export const REVIEW_PATCH_CLASS =
  "review-patch mt-3 flex min-h-90 flex-col overflow-hidden rounded-[var(--radius-sm)] border border-[var(--border)] bg-[#080706] [&>.loading-block]:m-auto [&>.empty-block]:m-auto [&>.notice]:m-auto [&>pre]:m-0 [&>pre]:flex-1 [&>pre]:overflow-auto [&>pre]:py-3.25 [&>pre]:text-xs [&>pre]:leading-[1.6] [&>pre]:text-[var(--text-soft)] [&>pre>code]:block [&>pre>code]:min-w-max";
...
const REVIEW_PATCH_LINE_BASE =
  "review-patch-line grid min-h-[1.6em] grid-cols-[28px_minmax(max-content,1fr)] pr-4.75 whitespace-pre [&>button]:sticky [&>button]:left-0 [&>button]:grid [&>button]:h-[1.6em] [&>button]:w-5.75 [&>button]:place-items-center [&>button]:border-0 [&>button]:border-r [&>button]:border-r-transparent [&>button]:bg-[color-mix(in_srgb,#080706_88%,transparent)] [&>button]:p-0 [&>button]:font-[var(--font-mono)] [&>button]:text-[11px] [&>button]:leading-none [&>button]:font-bold [&>button]:text-[var(--accent)] [&>button]:opacity-16 hover:[&>button]:border-r-[var(--accent-border)] hover:[&>button]:bg-[var(--accent-soft)] hover:[&>button]:opacity-100 [&>button:focus-visible]:border-r-[var(--accent-border)] [&>button:focus-visible]:bg-[var(--accent-soft)] [&>button:focus-visible]:opacity-100 [&>button.has-comment]:border-r-[var(--accent-border)] [&>button.has-comment]:bg-[var(--accent-soft)] [&>button.has-comment]:opacity-100 [&>button.has-comment]:after:absolute [&>button.has-comment]:after:top-0.75 [&>button.has-comment]:after:right-0.75 [&>button.has-comment]:after:size-1 [&>button.has-comment]:after:rounded-full [&>button.has-comment]:after:bg-[var(--accent)] [&>button.has-comment]:after:content-[''] [&>.review-patch-gutter]:sticky [&>.review-patch-gutter]:left-0 [&>.review-patch-gutter]:grid [&>.review-patch-gutter]:h-[1.6em] [&>.review-patch-gutter]:w-5.75 [&>.review-patch-gutter]:place-items-center";

export function reviewPatchLineClass(kind: string): string {
  if (kind === "addition")
    return `${REVIEW_PATCH_LINE_BASE} addition bg-[color-mix(in_srgb,var(--good)_9%,transparent)] text-[var(--good)]`;
  if (kind === "deletion")
    return `${REVIEW_PATCH_LINE_BASE} deletion bg-[color-mix(in_srgb,var(--bad)_9%,transparent)] text-[var(--bad)]`;
  if (kind === "hunk")
    return `${REVIEW_PATCH_LINE_BASE} hunk bg-[var(--accent-soft)] text-[var(--accent)]`;
  if (kind === "meta")
    return `${REVIEW_PATCH_LINE_BASE} meta text-[var(--faint)]`;
  return REVIEW_PATCH_LINE_BASE;
}
