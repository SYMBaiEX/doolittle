...
export const SETTINGS_LAYOUT_CLASS =
  "settings-layout grid min-h-0 flex-1 content-start items-start grid-cols-[190px_minmax(0,1fr)] gap-2.5 max-[1180px]:grid-cols-1 max-[1180px]:gap-2";

export const SETTINGS_NAV_CLASS =
  "settings-nav sticky top-0 grid max-h-full min-w-0 self-start content-start gap-1.5 overflow-auto pr-1.5 [scrollbar-gutter:stable] [scrollbar-width:thin] max-[1180px]:static max-[1180px]:flex max-[1180px]:max-h-none max-[1180px]:items-start max-[1180px]:gap-1.5 max-[1180px]:overflow-x-auto max-[1180px]:pr-0";
...
export const SETTINGS_NAV_GROUP_CLASS =
  "settings-nav-group min-w-0 [&>summary]:cursor-pointer [&>summary]:list-none [&>summary]:px-1.5 [&>summary]:py-0.75 [&>summary]:text-[length:var(--text-meta)] [&>summary]:font-semibold [&>summary]:text-[var(--muted)] [&>div]:grid [&>div]:content-start [&>div]:gap-px max-[1180px]:shrink-0 max-[1180px]:[&>summary]:hidden max-[1180px]:[&:not([open])>div]:grid max-[1180px]:[&>div]:grid-flow-col max-[1180px]:[&>div]:auto-cols-max max-[1180px]:[&>div]:items-center max-[1180px]:[&>div]:gap-1";

export const SETTINGS_NAV_BUTTON_CLASS =
  "relative flex min-h-7 w-full min-w-0 items-center rounded-[var(--radius-xs)] px-1.5 py-1 text-left text-[var(--text-soft)] hover:bg-[var(--surface-hover)] hover:text-[var(--text)] [&.selected]:bg-[color-mix(in_srgb,var(--accent)_7%,var(--surface-hover))] [&.selected]:text-[var(--text)] [&.selected]:shadow-[inset_2px_0_var(--accent)] max-[1180px]:w-auto max-[1180px]:shrink-0 [&>strong]:truncate [&>strong]:text-[length:var(--text-meta)]";

export const SETTINGS_CONTENT_CLASS =
  "settings-content grid min-h-0 min-w-0 content-start self-start gap-1.5";

export const SETTINGS_CONTENT_HEADER_CLASS =
  "settings-content-header flex min-h-8 items-center justify-between gap-3 pb-0.5 [&_h2]:mt-0 [&_h2]:mb-0.25 [&_h2]:text-sm [&_p]:m-0 [&_p]:text-[length:var(--text-meta)] [&_p]:text-[var(--muted)]";

export const SETTINGS_GROUP_CLASS =
  "settings-group grid min-w-0 content-start gap-2 rounded-[var(--radius-sm)] border border-[var(--line-subtle)] bg-[color-mix(in_srgb,var(--surface)_94%,var(--bg))] p-2.5";
