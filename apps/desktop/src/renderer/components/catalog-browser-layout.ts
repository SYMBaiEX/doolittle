...
export const CATALOG_INDEX_HEADER_CLASS =
  "flex min-h-[51px] items-center justify-between gap-3 border-b border-[var(--border)] bg-[color-mix(in_srgb,var(--surface-soft)_70%,transparent)] px-2.5 py-2 [&>div]:grid [&>div]:gap-px [&_small]:font-[var(--font-mono)] [&_small]:text-[var(--text-meta)] [&_small]:text-[var(--muted)] [&_strong]:text-[var(--text-control)] [&_strong]:text-[var(--text)]";
...
export const CATALOG_INDEX_TITLE_CLASS =
  "flex min-w-0 items-center justify-between gap-[7px] [&_strong]:overflow-hidden [&_strong]:text-ellipsis [&_strong]:whitespace-nowrap [&_strong]:text-[var(--text-control)]";
export const CATALOG_INDEX_META_CLASS =
  "flex min-w-0 items-center gap-[7px] font-[var(--font-mono)] text-[var(--text-meta)] text-[var(--muted)] [&>*]:overflow-hidden [&>*]:text-ellipsis [&>*]:whitespace-nowrap [&>:not(:last-child)]:after:ml-[7px] [&>:not(:last-child)]:after:text-[var(--faint)] [&>:not(:last-child)]:after:content-['·']";
export const CATALOG_INDEX_FOOTER_CLASS =
  "flex min-h-[42px] items-center justify-between gap-2.5 border-t border-[var(--border)] bg-[color-mix(in_srgb,var(--surface-soft)_66%,transparent)] px-2 py-1.5 [&>span]:font-[var(--font-mono)] [&>span]:text-[var(--text-meta)] [&>span]:text-[var(--muted)]";
...
export const CATALOG_FACTS_CLASS =
  "mt-3.5 grid border-t border-[var(--border)] [&>div]:grid [&>div]:min-w-0 [&>div]:grid-cols-[104px_minmax(0,1fr)] [&>div]:gap-3.5 [&>div]:border-b [&>div]:border-[var(--border)] [&>div]:py-2 max-[520px]:[&>div]:grid-cols-[82px_minmax(0,1fr)] max-[520px]:[&>div]:gap-[9px] [&_dd]:m-0 [&_dd]:min-w-0 [&_dd]:text-[var(--text-control)] [&_dd]:text-[var(--text-soft)] [&_dd]:[overflow-wrap:anywhere] [&_dt]:font-[var(--font-mono)] [&_dt]:text-[var(--text-meta)] [&_dt]:uppercase [&_dt]:text-[var(--muted)]";
export const CATALOG_TOKENS_CLASS =
  "mt-[17px] grid gap-[7px] [&>div]:flex [&>div]:flex-wrap [&>div]:gap-[5px] [&_code]:rounded-[var(--radius-xs)] [&_code]:border [&_code]:border-[var(--border)] [&_code]:bg-[var(--surface-soft)] [&_code]:px-1.5 [&_code]:py-1 [&_code]:text-[var(--text-meta)] [&_code]:text-[var(--text-soft)]";
