...
export function ChatTerminalPanel({
  ...
  return (
    <section
      ...
      className={`relative z-12 min-h-0 min-w-0 shrink-0 origin-bottom overflow-hidden border-t bg-[var(--canvas-bg)] transition-[height,opacity,transform,border-color,visibility] duration-200 ease-[cubic-bezier(0.16,1,0.3,1)] contain-[layout_paint_style] backface-hidden will-change-[transform,opacity] [@media(max-height:640px)]:max-h-[48vh] [&>[data-interactive-terminal]]:min-h-0 motion-reduce:transform-none motion-reduce:duration-[0.01ms] motion-reduce:delay-0 ${
        open
          ? "visible max-h-[58vh] translate-y-0 scale-y-100 border-[var(--border-strong)] opacity-100 shadow-[0_-12px_32px_color-mix(in_srgb,var(--shadow)_18%,transparent)] pointer-events-auto"
